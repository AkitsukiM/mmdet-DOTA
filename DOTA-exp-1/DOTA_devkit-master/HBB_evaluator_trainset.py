...
class HBB_evaluator(object):
    ...
    def __init__(self, work_root, pkl, dataset_root, nms_thresh = 0.3, mode = 0):
        ...
        # 验证集的原始标注txt文件
        self.annopath = self.dataset_root + r'DOTA/train/labelTxt/{:s}.txt'
        # 验证集的原始图片名文件
        self.imagesetfile = self.dataset_root + r'DOTA/train/trainset.txt'
        # 分割前的图片路径
        self.imagepath = self.dataset_root + r'DOTA/train/images/'
        # 分割后的图片路径
        self.split_imagepath = self.dataset_root + r'DOTA-ImgSplit/train/images/'
        # 分割后的json文件路径
        self.split_jsonpath = self.dataset_root + r'DOTA-ImgSplit-COCO/annotations/instances_train2017.json'
        # 分割后的标注txt文件
        self.split_annopath = self.dataset_root + r'DOTA-ImgSplit/train/labelTxt/{:s}.txt'
        ...
        self.train_coco = COCO(self.split_jsonpath) # 读取json文件
        self.result_all = pickle.load(open(self.resultPickle, 'rb')) # 读取pkl文件

        self.cats = self.train_coco.loadCats(self.train_coco.getCatIds())
        ...
        self.imgs = self.train_coco.loadImgs(self.train_coco.getImgIds())
