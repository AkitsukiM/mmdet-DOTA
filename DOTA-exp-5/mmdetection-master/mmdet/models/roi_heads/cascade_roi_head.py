...
class CascadeRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def aug_test(self, features, proposal_list, img_metas, rescale=False):
        ...
        # post scale mask #
        scale_ranges = rcnn_test_cfg.scale_ranges
        scale_i = 0
        ...
        for x, img_meta in zip(features, img_metas):
            ...
            # post scale mask #
            bboxes_S = (bboxes[:, 2] - bboxes[:, 0]) * (bboxes[:, 3] - bboxes[:, 1])
            min_S = scale_ranges[scale_i][0] ** 2
            max_S = scale_ranges[scale_i][1] ** 2
            bboxes_remove_mask = (bboxes_S < min_S) | (bboxes_S > max_S)
            scores[bboxes_remove_mask] *= 0.0
            scale_i += 1
