...
def bbox_to_rotated_box_np(bboxes):
    """
    :param bboxes: <class 'numpy.ndarray'> shape (n, 4) (xmin, ymin, xmax, ymax) or (n, 5) with score
    :return: dbboxes: <class 'numpy.ndarray'> shape (n, 5) (x_ctr, y_ctr, w, h, angle)
    """
    num_boxes = bboxes.shape[0]

    x_ctr = (bboxes[:, 2::5] + bboxes[:, 0::5]) / 2.0
    y_ctr = (bboxes[:, 3::5] + bboxes[:, 1::5]) / 2.0
    edges1 = np.abs(bboxes[:, 2::5] - bboxes[:, 0::5])
    edges2 = np.abs(bboxes[:, 3::5] - bboxes[:, 1::5])
    angles = np.zeros((num_boxes, 1))

    inds = (edges1 < edges2)[:, 0]

    if bboxes.shape[1] == 4:
        rotated_boxes = np.concatenate((x_ctr, y_ctr, edges1, edges2, angles), axis=1)
    # add score dim if exsists
    elif bboxes.shape[1] == 5:
        rotated_boxes = np.concatenate((x_ctr, y_ctr, edges1, edges2, angles, bboxes[:, 4::5]), axis=1)
    else:
        return ValueError('bboxes.shape[1] must be 4 or 5')

    rotated_boxes[inds, 2] = edges2[inds, 0]
    rotated_boxes[inds, 3] = edges1[inds, 0]
    rotated_boxes[inds, 4] = np.pi / 2.0
...
# NOTE
# 
# From https://github.com/csuhan/s2anet
# 
# 1. bbox_to_rotated_box_np
