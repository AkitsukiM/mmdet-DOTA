...
class ConvTransBlock(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, res_conv, stride, dw_stride, embed_dim, num_heads=12, mlp_ratio=4.,
                 ...
                 last_fusion=False, num_med_block=0, groups=1, proposal_token_num=None):

        super(ConvTransBlock, self).__init__()
        expansion = 4
        
        self.proposal_token_num = proposal_token_num
    ...
    def forward(self, x, x_t):
        ...
        # x_t = 1 + N + num_proposal(100)
        x_t_proposal_token = x_t[:, -self.proposal_token_num:]
        x_t = x_t[:, :-self.proposal_token_num]
        x_t = self.trans_block(torch.cat([x_st + x_t, x_t_proposal_token], dim=1))
        ...
        x_t = torch.cat([x_t, x_t_proposal_token], dim=1)
        return x, x_t


@BACKBONES.register_module()
class ConformerDet(BaseModule):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """

    def __init__(self, patch_size=16, in_chans=3, base_channel=64, channel_ratio=4, num_med_block=0,
                 embed_dim=768, depth=12, num_heads=12, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_eval=True, frozen_stages=0, out_indices=(4, 8, 11, 12),
                 num_stages=4, act_cfg=dict(type='GELU'), norm_cfg=dict(type='LN'), style='pytorch',
                 init_cfg=None, proposal_token_num=100):

        # Transformer
        super().__init__()
        
        self.proposal_token_num = proposal_token_num
        self.proposal_token = nn.Parameter(torch.zeros(1, self.proposal_token_num, embed_dim))
        trunc_normal_init(self.proposal_token, std=.02)
        ...
        for i in range(init_stage, fin_stage):
            self.add_module('conv_trans_' + str(i),
                            ConvTransBlock(
                                ...
                                proposal_token_num=self.proposal_token_num,
                            )
                            )
    ...
    def init_weights(self):
        ...
        if (self.init_cfg is None) or (self.init_cfg.checkpoint is None):
            ...
            load_checkpoint(self, self.init_cfg.checkpoint, strict=False, logger=logger, map_location='cpu')
    ...
    def forward(self, x):
        ...
        proposal_tokens = self.proposal_token.expand(B, -1, -1)
        ...
        x_t = torch.cat([cls_tokens, x_t, proposal_tokens], dim=1)
        ...
        for i in range(2, self.fin_stage):
            ...
            if i == self.out_indices[-1]:
                proposal_tokens = x_t[:, -self.proposal_token_num:]

        return tuple(output), proposal_tokens
    ...
    def train(self, mode=True):
        ...
        super(ConformerDet, self).train(mode)
        self._freeze_stages()
